...
from src.app.quiz_data import Quiz, QuizData
...
def check_quiz_answers(quiz: Quiz, user_answers: Dict[str, int], mappings: Dict[str, str]) -> Tuple[bool, Dict[str, bool]]:
    """
...
def get_display_variables(quiz: Quiz, 
                         quiz_data: QuizData, 
    ...
    Args:
        ...
        quiz_data: The quiz data containing Pokemon information
    ...
    display_vars = {
        ...
        for name, pokemon in quiz_data.pokemons.items()
    }
    
    # Add any special variable mappings
    for var, pokemon_name in variable_mappings.items():
        display_vars[var] = quiz_data.pokemons[pokemon_name].image_path
